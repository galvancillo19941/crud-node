...
routes.post('/login',(req, res) => {
  ...
  conn.query('SELECT * FROM users WHERE users.email = ? AND users.password', [email, password], ( error, results, fields) => {
    ...
    if (error) {
        ...
        const passMatch = bcrypt.compare(results[0].password);
        if (results.length > 0 && passMatch) {
            var tokenData = {
                email: email
              }
